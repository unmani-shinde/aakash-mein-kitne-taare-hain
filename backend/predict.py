...
model_name = "bigscience/bloom-560m"  # You can choose a different size based on your needs
model = AutoModelForCausalLM.from_pretrained(model_name)
tokenizer = AutoTokenizer.from_pretrained(model_name)

def generate_horoscope(prompt):
    inputs = tokenizer(prompt, return_tensors='pt', truncation=True, padding=True)
    outputs = model.generate(**inputs, max_length=3000, no_repeat_ngram_size=2, top_k=50, top_p=0.95, temperature=0.7)
    generated_text = tokenizer.decode(outputs[0], skip_special_tokens=True)
    return generated_text

# Function to generate a prompt based on blockchain activity
def generate_prompt(blockchain_activity):
    user_id = blockchain_activity.get("user_id", "N/A")
    transactions = blockchain_activity.get("transactions", [])
    transactions_summary = "\n".join(
        [f"  - Type: {txn.get('type')}, Amount: {txn.get('amount')}, Timestamp: {txn.get('timestamp')}" for txn in transactions]
    )
    interactions = blockchain_activity.get("interactions", [])
    interactions_summary = "\n".join(
        [f"  - Contract: {interact.get('contract')}, Action: {interact.get('action')}, Amount: {interact.get('amount')}, Timestamp: {interact.get('timestamp')}" for interact in interactions]
    )
    
    prompt = f"""
    Generate a personalized horoscope prediction based on the following blockchain activity. The activity summary is provided to help you understand the context of the user's recent transactions and interactions.
    ...
    Please provide a detailed and personalized horoscope based on the provided blockchain activity.
    """
...
# Generate the prompt
#prompt = generate_prompt(blockchain_activity)
batch_size = 10
results = []

for i in range(0, len(blockchain_activity), batch_size):
    batch = blockchain_activity[i:i + batch_size]
    for activity in batch:
        prompt = generate_prompt(activity)
        horoscope = generate_horoscope(prompt)
        results.append(horoscope)
        print(f"Generated horoscope for user {activity['user_id']}:\n{horoscope}\n")
...
output = model.generate(
    input_ids,
    max_length=5300,  # Adjust based on desired length
