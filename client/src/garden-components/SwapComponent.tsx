...
const MetaMaskButton: React.FC<MetaMaskButtonProps> = ({
  ...
  return (
    <button
    className={`btn btn-secondary ml-2 mr-2 mb-2 button-${isConnected ? "transparent p-2" : "transparent p-2"}`}
    onClick={onClick}
  >
    {buttonText}
  </button>
  ...
  return (
    <div className="swap-component-bottom-section">
      ...
      <div>
        ...
        <div className="input-component items-center">
          <input
            id="receive-address"
            className="input input-bordered w-full max-w-xs" 
      ...
      <button
        className={`btn btn-neutral button-${metaMaskIsConnected ? "white" : "black"}`}
        onClick={handleSwap}
